...
// State contains all mutable chip state for serialization.
// Variant-derived constants and audio config (gain, clocksPerSample) are
// excluded — they are reconstructed by the New constructor and SetGain,
// and the caller is expected to LoadState into an identically-configured
// instance. Callers saving full emulator state should persist gain separately
// via GetGain/SetGain if it has been changed from the default.
type State struct {
	ToneReg        [3]uint16
	ToneCounter    [3]uint16
	ToneOutput     [3]bool
...
// SaveState copies all mutable chip state into a State value and returns it.
...
// LoadState overwrites all mutable chip state from the provided State value.
// Resets bufferPos to 0 since the buffer contents are transient.
func (s *SN76489) LoadState(state State) {
	...
	s.noiseOutput = state.NoiseOutput
